...
public static class Utility
{
	...
	private static GameObject CopyActionsFromAndInitFirstChild(GameObject container, bool isInteractable, string agentTag)
	{
		...
		// functions
		foreach (Function funcCall in copyGO.GetComponentsInChildren<Function>(true))
		{
			Debug.Log("Fonction call");
		}

		foreach (FunctionInit funcInit in copyGO.GetComponentsInChildren<FunctionInit>(true))
		{
			Debug.Log("Fonction init");
			foreach (BaseElement act in funcInit.GetComponentsInChildren<BaseElement>(true))
			{
				if (!act.Equals(funcInit))
				{
					funcInit.firstChild = act.gameObject;
					break;
				}
			}
		}
	...
	public static void computeNext(GameObject container)
	{
		...
		if (container.transform.childCount > 0)
		{
			...
			if (parent != null && parent.GetComponent<BaseElement>() != null) {
				if (parent.GetComponent<ForControl>() != null || parent.GetComponent<ForeverControl>() != null || parent.GetComponent<FunctionInit>() != null)
